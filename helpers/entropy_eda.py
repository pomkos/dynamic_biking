...
def read_entropy_df(filename:str) -> pd.DataFrame:
    ...
    df = df[part_info +  [col for col in df.columns if col not in part_info]] # reorganize
    return df
...
def app(filename:str):
    ...
    with colx:
        x = st.selectbox("X axis", options=df.columns)
    with coly:
        y = st.selectbox("Y axis", options=df.columns, index=10)
    ...
    graph = st.selectbox("Graph type", options=graph_options, index=default, format_func=format_options)
